...
class GameBot:
    def __init__(self):
        ...
        print(
            f"视觉特征维度: {self.vision.detect(np.zeros((1600, 2560, 3), dtype=np.uint8))['features'].shape}"
        )
        print(f"内存输入维度: {self.memory.feature_encoder[0].in_features}")
        print(
            f"决策网络输入维度: {self.decision_system.decision_net.encoder[0].in_features}"
        )
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    if (
                        objects.get("enemies")
                        and len(objects["enemies"]) > 0
                        and self.combat_time <= 0
                    ):
                    ...
                    path = self.planner.plan(
                        current_pos, objects["enemies"], memory_data
                    )
                    ...
                    self.memory.agent_direction = (
                        new_dir  # 只储存方向角度，不储存元组（还有个俯仰角不需要）
                    )
                    ...
                    cycle_data = {
                        ...
                        "current_map": self.memory.env_map.detach()
                        .cpu()
                        .numpy()
                        .copy(),
                    ...
                    self.decision_system.record_experience(
                        ...
                        action,
                        reward,
                        self.decision_system.prepare_input(
                            self.memory, objects["features"], self.controller
                        ),
                        False,  # 假设不会结束
                    )
                    ...
                    if self.step_count % self.train_interval == 0:
                        ...
                        # 调整神经网络影响力
                        self.decision_system.adjust_influence_weight(
                            self.recent_rewards
                        )
                        print(
                            f"更新神经网络影响力权重: {self.decision_system.nn_influence:.2f}"
                        )
    ...
    def print_status(self):
        ...
        avg_detections = (
            np.mean(self.detection_counts[-10:]) if self.detection_counts else 0
        )
    ...
    def _handle_combat(self, enemies):
        ...
        if len(enemies) > 0:
            ...
            closest_enemy = min(
                enemies,
                key=lambda e: (e[0] - screen_center[0]) ** 2
                + (e[1] - screen_center[1]) ** 2,
            )
